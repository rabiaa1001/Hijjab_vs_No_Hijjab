...
def load_image_and_resize(image_file: str, width: int, height: int):
    ...
    try:
        ...
        if not isinstance(width, int) and isinstance(height, int):
            raise ValueError(
                "Expecting width and height values to be integers e.g 224, 224 ")
...
def single_image(single_image: Image):
    ...
    st.markdown(
        "<h4 style='text-align: left; color: gray;'>Model Identified this woman as having : </h4>",
...
def main():
    ...
    # Banner pic
    st.image(
        os.path.join('../web_images/hijjab_banner.png'),
        use_column_width='auto')

    # Sidebar Image and Texts
    st.sidebar.image(
        os.path.join('../web_images/together.jpg'),
    ...
    uploaded_image = st.sidebar.file_uploader(
        'Upload your portrait here', type=[
            'jpg', 'jpeg', 'png'])
    if uploaded_image is not None:
        ...
        # Send image to test_on_single_image function to predict whether or not
        # the woman is wearing a hijjab
        single_image(
            load_image_and_resize(
                uploaded_image,
                IMAGE_WIDTH,
